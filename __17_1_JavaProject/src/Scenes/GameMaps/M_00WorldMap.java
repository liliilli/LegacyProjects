...
public class M_00WorldMap extends AbstractScene {
    private boolean flag = false;

    public M_00WorldMap(String path) throws IOException, FontFormatException {
        super(path);
    ...
    public void activate() {
        ...
        System.out.println(mContainer.getMapName());

        PanelHandler.handle(PLAY_STATUS, getMapContainer().getPlayerObject().getStatus());
    }
    ...
    public boolean draw() {
        ...
        PanelHandler.getInstance().getPanel(PLAY_STATUS).revalidate();
        PanelHandler.getInstance().getPanel(PLAY_STATUS).repaint();
        return false;
    }
    ...
    public void resolveInformationWithIntent(Intent intent) {
        if ((Boolean)intent.get("IsPlayerExist")) {
            ...
            getCharacterContainer(ContainerType.PLAYER).add(
                    CharacterType.create(CharacterType.PLAYER,
