package Game.Objects.Characters.Controllable;
...
public class Player extends AbstractCharObject {
    public Player(int x, int y, String name, MapInfoContainer c) {
        super(x, y, c);
        mName = name;
    ...
    private void interact() {
        ...
        status.addExp(new Random().nextInt(5));
    ...
    protected void battle(AbstractCharObject my,
        ...
        Game.getInstance().playSound(SoundsList.SWORD);
        ...
        if (enemy.getStatus().isDead()) {
            int exp = enemy.getStatus().getExperience();
            status.addExp(exp);

            // 적을 죽인다.
            displayLog( enemy.getName() + " is vanished to the ash." );
            displayLog( getName() + " is earned " + exp
                    + " Experience point" + "." );
            Game.getInstance().playSound(SoundsList.MONSTER_DEAD);
