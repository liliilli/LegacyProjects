...
class Item {
public:
    Item(int id, const int* const pos) : id(id), pos(pos) {};
    Item() : id(-1), pos(new int[2]) {};
    ~Item() { delete[] pos; }
...
void initialize_items(Cluster clusters[], Item items[]);
...
bool check_cluster_hit_limit(Item items[], int size, int limit);
...
int main() {
    ...
    cin >> item_num >> dim; // Dimension and The number of items
    cluster_size = item_num + 1;

    // Input and Create Item Informations
    Item* items = new Item[cluster_size];
    Cluster* clusters = new Cluster[cluster_size];
    initialize_items(clusters, items);
    ...
    for (int i = 0; i <= item_num; ++i) { distances.push_back(&clusters[i]); }
    ...
    while (!check_cluster_hit_limit(items, item_num, LIMIT)) { // at least loop N - 1 times
        ...
        if (y_root != x_root) {
            ...
            // 대상 Cluster 을 지운다.
            Cluster* temp_clusters = new Cluster[cluster_size - 1];
            int j = 0;
            for (int i = 0 ; i < cluster_size; ++i) {
                if (i != y_id && i != x_id) {
                    temp_clusters[j] = clusters[i]; 
                    j++;
                } 
            }
            temp_clusters[j] = cluster;
            --cluster_size;
            ...
            for (int i = 0; i < cluster_size; ++i) { 
                distances.push_back(&clusters[i]); 
            }
...
void initialize_items(Cluster clusters[], Item items[]) { // Make Set
    for (int i = 1; i <= item_num; ++i) {
        int* position = new int[dim];
        for (int j = 0; j < dim; ++j) { int value = 0; cin >> value; position[j] = value; }

        Item item(i, position); item.cluster = i;
        items[i] = item;
