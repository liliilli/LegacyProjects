...
/////////////////////////////////////////////
// MERGE SORT
void merge_sort(int array[], int buf[], int s, int e) {
  if (s == e) return;

  // Devide
  int sep = (s + e) >> 1;
  merge_sort(array, buf, s, sep);
  merge_sort(array, buf, sep + 1, e);
...
void mergesort(int A[], int buf[], int N) {
  merge_sort(A, buf, 0, N - 1); // look forward to Tailing recursion optimization 
}
/////////////////////////////////////////////
...
/////////////////////////////////////////////
// QUICK SORT
int get_partition(int array[], int s, int e) {
  int i_pre = s - 1;
  for (int i = s; i < e; ++i) {
    if (array[i] < array[e]) {
      i_pre += 1;
  ...
  i_pre += 1;
  SWAP(int, &array[i_pre], &array[e]);
  return i_pre;
}

void quick_sort(int array[], int s, int e) {
  if (s < e) { // Randomization for worst case input array.
    int r_index = (rand() % (e - s + 1)) + s;
    SWAP(int, &array[r_index], &array[e]);

    int sep = get_partition(array, s, e);
    quick_sort(array, s, sep - 1);
    quick_sort(array, sep + 1, e);
  }
}
